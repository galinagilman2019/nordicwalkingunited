...
class EventApplicationManager(models.Manager):
    def create(self, **data_dict):
        ...
        nwgroup = data_dict["nwgroup"]
        logger.debug(f"main_organizer_email: {main_organizer_email}")
        logger.debug(f"nwgroup: {nwgroup}")
        try:
        ...
        previous_application = (
            EventApplication.objects.filter(main_organizer_email=main_organizer_email, status=NEW, nwgroup=nwgroup)
            .order_by("-created_at")
            .first()
        )
        logger.debug(f"previous_application: {previous_application}")
...
#            ) < timedelta(days=180):
        ...
        previous_event = (
            EventApplication.objects.filter(main_organizer_email=main_organizer_email, status=DEPLOYED, nwgroup=nwgroup)
            .order_by("-date")
            .first()
        )
...
class EventApplication(models.Model):
    previous_event = models.ForeignKey(Event, blank=True, null=True, on_delete=models.deletion.SET_NULL, related_name="event_applications_organize" )
    # workshop fields
    ...
    involvement = models.CharField(_("Involvement in Nordic Walk United"), max_length=100)
    ...
    def create_event(self):
        logger.debug(f" CREATE EVENT : ")
        """Creates event based on the data from the EventApplication."""
        logger.debug(f" create_event self.nwgroup: {self.nwgroup}")
        logger.debug(f" create_event self.city: {self.city}")
        name = f"{self.nwgroup} {self.city}"
        logger.debug(f" create_event.name: {name}")
        email = f"{self.website_slug}@djangogirls.org"

        event = Event.objects.create(
            date=self.date,
            city=self.city,
            nwgroup=self.nwgroup,
    ...
    def deploy(self):
        ...
        """
        logger.debug("MODELS Entering deply.")
        ...
        logger.info(f"MODELS main_organizer: {main_organizer}")
        event.main_organizer = main_organizer
        logger.info(f"MODELS event with main_organizer: {event}")
        event.save()
        # add all co-organizers
        for organizer in self.coorganizers.all():
            logger.info(f"MODELS in self.coorganizers.all() organizer: {organizer}")
            event.add_organizer(organizer.email, organizer.first_name, organizer.last_name)
            logger.info(f"MODELS in self.coorganizers.all() event: {event}")
        return event
